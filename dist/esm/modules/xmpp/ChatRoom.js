...
export default class ChatRoom extends Listenable {
    ...
    setAffiliation(jid, affiliation) {
        const grantIQ = $iq({
            ...
            .up().up().up();
        this.connection.sendIQ(grantIQ, result => logger.info('Set affiliation of participant with jid: ', jid, 'to', affiliation, result), error => logger.error('Set affiliation of participant error: ', error));
    }
    /**
     *
     * @param jid
    ...
    kick(jid, reason = 'You have been kicked.') {
        ...
        this.connection.sendIQ(kickIQ, result => logger.info('Kick participant with jid: ', jid, result), error => logger.error('Kick participant error: ', error));
    }
    /* eslint-disable max-params */
    /**
     *
    ...
    muteParticipant(jid, mute, mediaType) {
        ...
        const iqToFocus = $iq({ to: this.focusMucJid,
            ...
            .t(mute.toString())
            .up();
        this.connection.sendIQ(iqToFocus, result => logger.info('set mute', result), error => logger.error('set mute error', error));
    }
    /**
     * TODO: Document
     * @param iq
